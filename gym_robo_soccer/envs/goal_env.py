...
class GoalEnv(gym.Env):
    ...
    def reset(self):
        ...
        initial_goalie = self._keeper_starting_position(initial_ball, PITCH_LENGTH / 2)
        opponent_player_pos = self._keeper_starting_position(initial_ball, PITCH_START)
        angle2 = angle_between(initial_goalie, initial_ball)
        angle3 = angle_between(opponent_player_pos, initial_ball)
    ...
    def _keeper_starting_position(self, ball, goal_line_position):
        grad, yint = self._keeper_line(ball)
        xval = goal_line_position
        return np.array(( xval, GOAL_AREA_WIDTH / 32 ))
    ...
    def __draw_internal_state(self, internal_state, fade=False):
        ...
        left_goal_line_pos = self.__visualiser_scale(PITCH_START)

        # Right Goal Box
        self.__draw_vertical(length, 0, width)
        self.__draw_box(PITCH_LENGTH / 2 ,GOAL_AREA_WIDTH, GOAL_AREA_LENGTH)

        # Center Line
        self.__draw_vertical(length / 2, 0, width)
        
        # Left Goal Box
        self.__draw_vertical(left_goal_line_pos, 0, width)
        self.__draw_box(PITCH_START, GOAL_AREA_WIDTH, -GOAL_AREA_LENGTH)

        # self.draw_box(PENALTY_AREA_WIDTH, PENALTY_AREA_LENGTH)

        depth = length + self.__visualiser_scale(GOAL_DEPTH)
        depth_left_box = left_goal_line_pos + self.__visualiser_scale(GOAL_DEPTH)
        self.__draw_horizontal(width / 2 - self.__visualiser_scale(GOAL_WIDTH / 2), length, depth)
        self.__draw_horizontal(width / 2 + self.__visualiser_scale(GOAL_WIDTH / 2), length, depth)

        self.__draw_horizontal(width / 2 - self.__visualiser_scale(GOAL_WIDTH / 2), left_goal_line_pos, -depth_left_box)
        self.__draw_horizontal(width / 2 + self.__visualiser_scale(GOAL_WIDTH / 2), left_goal_line_pos, -depth_left_box)
        ...
        self.__draw_player(opponent_goalie_position, opponent_goalie_orientaion, self.__red)
        if not fade:
            self.__draw_radius(opponent_goalie_position, CATCHABLE)
    ...
    def __draw_box(self, start_position, area_width, area_length):
        ...
        line = self.__visualiser_scale(start_position - area_length)
        self.__draw_vertical(line, lower_corner, upper_corner)
        self.__draw_horizontal(lower_corner, line, self.__visualiser_scale(start_position))
        self.__draw_horizontal(upper_corner, line, self.__visualiser_scale(start_position))
    ...
    def __draw_player(self, position, orientation, colour):
        ...
        # if (colour == self.__black):
            # pygame.draw.line(self.window, self.__red, pos, end)
        pygame.draw.line(self.window, self.__green, pos, end)
    ...
    def _initialse_window(self):
        ...
        if self.window is None:
            ...
            self.__green = pygame.Color(0, 125, 0, 0)
